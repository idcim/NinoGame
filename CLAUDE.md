...
**Stage 3b3 (v0.5.7, 已落)**:
- `RequestDialog` Compose AlertDialog: 多行文本输入 + 发送按钮 → `AgentService.sendUnlockRequest(text)` 静态方法发 `{type:unlock_request, payload:{request_text, structured:{}}}`. 协议跟 Windows agent `ui/request_dialog.py` 一致, server `onUnlockRequest` 转家长浏览器审批; 批准后 server 推 `command(temporary_unlock)`, Stage 3b1 链路自动放行规则.
- Dashboard 加"申请玩游戏…"按钮 (仅 Child 模式显示, Lock / Parent 不展示, 防误点). 发送成功后 SnackbarHost 浮出"已发送, 等家长回".
- `ScreenStateReceiver` (BroadcastReceiver, **运行时注册** 不能 Manifest 声明 — Android 安全限制): 监 `ACTION_SCREEN_OFF` / `ACTION_SCREEN_ON` / `ACTION_USER_PRESENT`.
  - SCREEN_OFF → 起 5min 定时协程, 还在 Child 模式就切 Lock
...
- [x] LLM 后台分类器 (unknown apps): Agent 周期推 unknown_apps → server LLM 分类 → app_categories_update 推回 Agent 写本地缓存
...
- [x] ~~每日总结推送 (P4 外补)~~ (v0.4.7 完成): server 端 `daily_summary_scheduler` 每分钟扫一次本地时间, 命中 `admin_settings.daily_summary.time` (默认 21:00, opt-in via .enabled) 时给每位 parent 的每个有今日活动的 child compose 一条今日摘要 (active 时长 + token 净变化 + Top 1 应用) 走 v0.4.1 notifier (企微 + SMTP, info 级). dedupe_key=`daily_summary:CHILD_ID:YYYY-MM-DD` 防同日重发, 0 active 跳过. 闭环 §15 家长可见性 — 不用打开浏览器就知道"今天 Nino 怎么样了". 详见 `backend/src/services/daily_summary_scheduler.ts`.
- [x] ~~屏幕使用时长统计（不是截屏，仅前台时间）~~ (v0.4.5 完成): /reports 加 `/reports/category-breakdown` 端点 + CategoryBreakdownCard — `app_sessions.category` 桶聚 (复用 GRANULARITY_CONFIG), 三类横向 bar (消遣 warn / 学习 accent / 中性 brand) + 时长 + % 占比. 纯描述性, 不参与扣分决策 (决策 #33 后 category 不影响 token), 但仍是"时间花在哪"的有效信号 — 让家长一眼看出"是不是 80% 时间都在消遣类". 配 `api.getCategoryBreakdown`, 跟 granularity+periods 联动.
...
### P6 — 题库自动赚分主线（远期大主线 / 跨端）

**背景：** 决策 #33 下线了"前台学习应用自动检测 +token"路径，孩子目前只能靠 Path 2/3/5（家长私下确认 / 任务 / 酌赠）赚分。这条路径有摩擦、依赖家长在场。**P6 用"答对题目 → 自动 +token"补全自动赚分能力**，让孩子在家长不在场时也能挣到 token，但与下线的 Path 1 不同——这次是**主动答题**而非被动检测，孩子有明确意图、系统有明确判定，符合 §1.3"协商接口而非控制器"哲学。

**核心设计原则：**

- **按年龄匹配题库**：家长在后台设定孩子年龄段（或绑定 birth_year），系统从题库自动出对应难度的题（如 6-8 岁口算、9-12 岁应用题、13+ 拓展题等）。
- **答对得分有上限**：每日答题挣分上限（防止刷题刷上限）、每题冷却（防止反复答同一题）、错题不扣分但要重试/换题。
- **题库自身可扩展**：先实装一个内置基础题库（数学优先），后期加学科插件、家长自定义、第三方题库接入。
- **作为独立子模块**：题库与 NinoGame 主体解耦，可作为"插件"或"公共库"独立维护、独立分发。家长后台的"题库管理"是独立模块，可关闭可启用。
- **跨端兼容（核心约束）**：手机 + PC 都要能答题。优先 PWA / 响应式 Web 方案而非各做原生 App。这是从设计开始就要规划的形态，不是后期适配。

**功能点：**

- [ ] 题库 schema 设计（题型 / 学科 / 难度 / 年龄段 / 答案与判分 / 解析）
- [ ] 内置基础题库（数学优先，覆盖 6-15 岁分段）
- [ ] 家长后台「题库管理」：浏览、按年龄筛选、启用/禁用、查看孩子答题历史
- [ ] 孩子端答题 UI（响应式 Web，PC + 手机一套代码）
- [ ] 每日答题挣分上限 + 单题冷却 + 错题处理策略
- [ ] 与 token 经济集成（答对 → server 写 ledger reason='quiz_reward'）
- [ ] 题库扩展接口（家长自定义题 / 第三方题包导入）
- [ ] 防作弊（防搜索引擎复制粘贴 / 防多账号刷分 / 抽样验证）

**待决策项（开工前必谈）：**

- 题库主战场是网页（PWA）还是嵌入 Agent / Android App？
- 家长自定义题 vs 第三方专业题库（优先级 / 是否做内容审核）
- 答题失败的反馈方式（直接显示答案 / 给提示让再试 / 累积错题集）
- 与"任务系统"（§8.3 Path 3）的关系：题库是一种特殊任务模板，还是独立子系统？

**约束：**

- 不破坏现有 token 经济模型（决策 #33-#36 仍然成立，只是新增一条入账 reason）
- 不偷偷给孩子加分——所有 +token 仍以家长可见的 ledger 条目呈现
- 不让题库变成"应试加压工具"——首版目标是"让孩子有动力做点动脑的事换游戏时间"，不是替代学校作业
...
| 44   | 跨端 CHANGELOG 单一真相 | **CHANGELOG.md 在 repo 根 (v0.4.9 / v0.5.17)**: backend `/api/changelog` 公开端点读 (60s 缓存) → admin "更新日志" 页 + Android Settings/AboutDialog "查看更新日志" 都拉同一份, Win agent About 下一站也接 (Task #78). docker-compose volume `../CHANGELOG.md:/app/CHANGELOG.md:ro` 挂进 backend 容器, 改 markdown 不用 rebuild 镜像. **理由**: 之前发版只在 git commit message 里写, 用户/家长看不到; "版本有更新就更新一下后台和各 agent 的更新日志" 用户明确要求. 单一真相源避免三端各自维护文案漂移. | §21 + v0.4.9 / v0.5.17 |
