...
```
启动消耗类应用 → 实时扣 token（按分钟计）
```

- 默认 1 token = 1 分钟
- 应用费率系数：标准 1x、高消耗 1.5-2x（按档位）
- 每日硬上限：不论 token 多少，达到即停（按档位）
...
**机制：** 生产类应用前台时长达到阈值，自动写 token ledger。

| 类别           | 示例                       | 默认奖励                          |
| -------------- | -------------------------- | --------------------------------- |
| 阅读类         | Kindle、微信读书、得到     | 30min = +30 token，每日上限 60min |
| 学习类         | Duolingo、可汗学院         | 30min = +25 token                 |
| 创作类         | VSCode、Scratch、Procreate | 30min = +20 token                 |
| 健身类（手机） | Keep、Nike Training        | 完成一节 = +30 token              |

**关键规则：**

- 必须前台聚焦 + 用户活跃（详见 §16 防刷）
- 切到后台或闲置 >2 分钟立即停止计时
- 跨端聚合（PC + 手机的同类活动累加，按每日上限封顶）

### 8.2 Path 2: 一键申报（低摩擦）

| 事项       | 申报方式          | 验证                |
| ---------- | ----------------- | ------------------- |
| 作业完成   | 文字 + 作业本拍照 | LLM 初筛 → 家长批准 |
| 户外运动   | 截图运动 App 数据 | 多数自动通过        |
| 家务       | 拍前后对比照      | LLM 看图 → 家长批准 |
| 乐器练琴   | 录 30s 视频       | 家长听后批准        |
| 看了一本书 | 写 50 字读后感    | 家长读后批准        |
...
for each 5-minute window:
    for each foreground_segment:
        if category == 'consumption' and user_active:
            deduct = seconds / 60 * rate_multiplier
            charge(wallet, deduct)
        elif category == 'productive' and user_active and within_daily_cap:
            award_path1(wallet, ...)
        else:
            # neutral / idle / unlock_period: skip
            pass
```

**关键约束：** 只有"active + consumption"扣分，"active + productive"赚分。其他状态都不影响 token。

### 10.3 用户活跃判定

| 场景                     | 判定标准                                             | 频率          |
| ------------------------ | ---------------------------------------------------- | ------------- |
| 消费计费的"活跃"         | 最近 2 分钟内有键鼠输入                              | 每 5 分钟检查 |
| 赚分判定的"活跃"         | 最近 60 秒内有 key/scroll/click 事件（鼠标位移不算） | 每 60 秒检查  |
...
def translate_request(text, context):
    ...
    return {
        ...
        "llm_summary": "孩子声称完成作业，申请30分钟。本周基础时长还剩100分钟。建议要求作业拍照确认。",
        "confidence": 0.92
    }
```

家长端看到的是干净的结构化卡片 + 一行 LLM 总结 + 三个按钮（批准/要照片/拒绝）。
...
家长要求 Nino 拍照证明 → Nino 上传 → LLM Vision 初筛：

```
"看起来是数学作业，三页，最后一题答案可能有误。基本看起来是完整的。"
```

家长决策时间从"看半分钟照片"降到"看 5 秒摘要"。
...
家长点：批准 / 要照片 / 拒绝 / 谈谈
   ↓
[若要照片] Backend → Agent: request_photo
   ↓ Nino 拍照上传
   ↓ LLM 看图 → 摘要给家长
   ↓ 家长终审
   ↓
...
CREATE TABLE task_templates (
  ...
  verification VARCHAR(16),      -- parent_approve / photo / self_report / auto
...
| `request_photo`    | `{purpose}`                                  | 要求孩子端弹拍照框      |
