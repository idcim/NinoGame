...
const SYSTEM_PROMPT = `你是 NinoGame 应用分类助手。把孩子在 Windows / Android 上前台运行的应用归类。
...
输出: {"category":"consumption","sub_type":"game","confidence":0.99,"reasoning":"PvZ 桌面游戏"}

输入: app=Code.exe; window=main.py - Visual Studio Code
输出: {"category":"productive","sub_type":"code","confidence":0.98,"reasoning":"VSCode 代码编辑器"}

输入: app=chrome.exe; window=Google
输出: {"category":"neutral","sub_type":"browser","confidence":0.95,"reasoning":"Chrome 浏览器, 用途中性"}

输入: app=Bilibili.exe; window=B站直播
输出: {"category":"consumption","sub_type":"video","confidence":0.92,"reasoning":"B 站视频客户端"}

输入: app=KindleForPC.exe; window=Kindle - 第三章
输出: {"category":"productive","sub_type":"reading","confidence":0.97,"reasoning":"Kindle 桌面阅读应用"}

输入: app=unknown_proc.exe; window=
输出: {"category":"neutral","sub_type":"system","confidence":0.3,"reasoning":"无窗口标题, 倾向中性兜底"}`;
...
function normalize(obj: Record<string, unknown>): LlmCategory {
  ...
  return { category: cat, sub_type: subType, confidence: conf, reasoning };
}
